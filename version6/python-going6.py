...
class ReverseShellDetector(AnomalyDetector):
    def detect(self, socket: Socket, history: Dict, config: MonitorConfig) -> Optional[Anomaly]:
        reverse_shell_ports = {4444, 1337, 31337, 1234, 4321, 9999}
        suspicious_processes = {"bash", "sh", "zsh", "python", "perl", "nc", "netcat"}
        
        if (socket.remote_port in reverse_shell_ports and
            socket.state == "ESTABLISHED"):
            process_name = socket.process.split()[0].lower() if socket.process else "unknown"
            if process_name in suspicious_processes:
                return Anomaly(
                    ...
                    socket=socket,
                    reason=f'Shell process connecting to common reverse shell port {socket.remote_port}',
...
class ReportExporter:
    @staticmethod
    def export_json(anomalies: List[Anomaly], filename: str):
        data = {
            'anomalies': [
                {
                    'type': a.type,
    ...
    @staticmethod
    def export_csv(anomalies: List[Anomaly], filename: str):
        with open(filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Type', 'Severity', 'Score', 'Reason', 'Process', 'Remote', 'Timestamp'])
            ...
            for anomaly in anomalies:
                writer.writerow([
                    ...
                    f"{anomaly.score:.2f}",
                    anomaly.reason,
                    anomaly.socket.process,
                    f"{anomaly.socket.remote_ip}:{anomaly.socket.remote_port}",
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(anomaly.timestamp))
                ])
...
class NetworkMonitor:
    ...
    def export_anomalies(self, anomalies: List[Anomaly], format: str, filename: str):
        if format == 'json':
            self.report_exporter.export_json(anomalies, filename)
        elif format == 'csv':
            self.report_exporter.export_csv(anomalies, filename)
        else:
...
Examples:
  ...
  python network_monitor.py --export-anomalies json --export-file report.json
        """,
    ...
    output_group.add_argument("--export-anomalies", choices=["json", "csv"], help="Export anomalies to file")
    output_group.add_argument("--export-file", type=str, default="anomalies_report", help="Export filename")
    ...
    def process_cycle():
        ...
        if args.export_anomalies and anomalies:
            filename = f"{args.export_file}.{args.export_anomalies}"
            monitor.export_anomalies(anomalies, args.export_anomalies, filename)
            print(f"Anomalies exported to {filename}")
